...
ほとんどの求人が英語の場合は、デフォルトの `modes/` を使ってください。英語モードでもエージェントが日本語の求人を検出すれば自動的に対応しますが、日本市場の細かい慣習までは同じレベルで把握していません。
...
エージェントは `modes/` の代わりにこのフォルダのファイルを読み込みます。
...
最初のセッションでエージェントにこのフィールドを尊重するよう伝えてください（「`profile.yml` を見て、`language.modes_dir` を設定してある」）。以降、エージェントは自動的に日本語モードを使用します。
